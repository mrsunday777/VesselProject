...
## Pending Actions
- [ ] **🚨🚨🚨 BRANDON DIRECTIVE — RE-SPAWN FULL CREW NOW (Feb 6 ~16:15)**
  - Relay was restarted to fix display mapping bugs. All sessions were wiped.
  - All spawn gates are still AUTHORIZED. Vessel is connected. Relay is live.
  - **Re-spawn all agents immediately** with their previous assignments:
    1. `wallet.spawn_agent("msSunday", "content_manager", "<your content manager prompt>", max_turns=20)`
    2. `wallet.spawn_agent("msCounsel", "compliance_counsel", "<your compliance prompt>", max_turns=20)`
    3. `wallet.spawn_agent("CP0", "trader", "<your trader prompt>", max_turns=20)`
    4. `wallet.spawn_agent("CP1", "scanner", "<your scanner prompt>", max_turns=20)`
    5. `wallet.spawn_agent("CP9", "health_monitor", "<your health monitor prompt>", max_turns=20)`
    6. `wallet.spawn_agent("Chopper", "scout", "<your scout prompt>", max_turns=10)`
  - Chopper is NOW UNBLOCKED at the relay — gate workspace + availability added.
  - Display mapping fixes are live: health_monitor→Health, scanner→Scanning, compliance→Compliance
...
- Agents run on the **phone vessel** — NOT on Mac. No `openclaw` processes.
